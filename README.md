...
4. 其余配置保持默认，生成令牌并完整复制
5. Store token safely, it only shows once
5. 妥善保存令牌，页面仅展示一次，刷新后无法查看
...
1. 切换至 Actions 面板，选择工作流：第一步 首次证书初始化签发
...
- `fullchain.pem`: 完整证书链，网站HTTPS部署通用文件
- `private.key`: Domain private key, keep confidential
- `private.key`: 域名私钥，请勿对外泄露
...
## 🎬 作者哔哩哔哩账号
Check our channel for related tutor
